        ...
        if (newVerifiedEmail) {
            transporter.sendMail(mailOptions)
                .then(() => {
                    res.status(201).send({
                        success: 'PENDING',
                        message: 'Verification email has been send to your account.  Check your email for further instructions.'
                    })
                })
                .catch((err) => {
                    console.log(err);
...
export const resetPassworLink = async (user, res) => {
    ...
    try {
        const hashedToken = await hashString(token)
        const newResetpassword = await PasswordReset.create({
            userId: _id,
            email: email,
            token: hashedToken,
            createdAt: Date.now(),
